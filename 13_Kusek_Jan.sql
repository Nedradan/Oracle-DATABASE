...
--
------------------------
	-- FUNKCJA pomocnicza f_SZA_OPIS_OPERACJI
	------------------------
create or replace function f_SZA_OPIS_OPERACJI(
							f__SZ_KAT_ID in SZACHOWE_LICENCJE.KAT_ID%TYPE, 
							f_SZ_ZWI_ID in SZACHOWE_LICENCJE.ZWI_ID%TYPE,
							f_SZ_OSO_ID in SZACHOWE_LICENCJE.OSO_ID%TYPE)
							--DEFINIUJE TRYB ZWRACANY
							return varchar2
is
	--TWORZENIE KURSORA
	CURSOR kursor_SZACHOWE_LICENCJE(k__SZ_KAT_ID in SZACHOWE_LICENCJE.KAT_ID%TYPE, 
							k_SZ_ZWI_ID in SZACHOWE_LICENCJE.ZWI_ID%TYPE,
							k_SZ_OSO_ID in SZACHOWE_LICENCJE.OSO_ID%TYPE)
	is
		select * from SZACHOWE_LICENCJE 
		where KAT_ID = k__SZ_KAT_ID and 
			ZWI_ID = k_SZ_ZWI_ID and
			OSO_ID=k_SZ_OSO_ID;
			
			
	--TWORZENIE ZMIENNEJ O TYPIE WIERSZOWYM
	wiersz kursor_SZACHOWE_LICENCJE%ROWTYPE;
	--TWORZENIE ZMIENNEJ POMOCNICZEJ
	opis varchar2(40);
...
begin
	--OTWARCIE KURSORA
	open kursor_SZACHOWE_LICENCJE(f__SZ_KAT_ID,f_SZ_ZWI_ID,f_SZ_OSO_ID);
	--PETLA
	loop 
	--PRZESZUKIWANIE KURSORA I ZAPISYWANIE WYNIKOW DO ZMIENNEJ WIERSZ
	fetch kursor_SZACHOWE_LICENCJE into wiersz; 
		-- warunek wyjścia z pętli
		exit when kursor_SZACHOWE_LICENCJE%NOTFOUND or kursor_SZACHOWE_LICENCJE%ROWCOUNT <1;
		--TWORZENIE OPISU
		opis :=	'OSO:'||wiersz.OSO_ID||
				'/ZWI:'||wiersz.ZWI_ID||'/KAT:'||wiersz.KAT_ID;
	end loop;
	--ZAMKNIECIE KURSORA
	close kursor_SZACHOWE_LICENCJE;
	commit;
	--ZWROCENIE ZMIENNEJ OPIS
	return opis;
exception
	when OTHERS then
		DBMS_OUTPUT.PUT_LINE('Nieokreslony błąd w procedurze f_SZA_OPIS_OPERACJI!');
...
------------------------
-- REKORD DEFINIOWANE PRZEZ PROGRAMISTĘ
------------------------	
create or replace procedure p_UPorINS_SZA_LICENCJE(
							p__SZ_KAT_ID in SZACHOWE_LICENCJE.KAT_ID%TYPE, 
							p_SZ_ZWI_ID in SZACHOWE_LICENCJE.ZWI_ID%TYPE,
							p_SZ_OSO_ID in SZACHOWE_LICENCJE.OSO_ID%TYPE,
							p_nowa_KAT_ID in SZACHOWE_LICENCJE.KAT_ID%TYPE,
							p_nowa_ZWI_ID in SZACHOWE_LICENCJE.ZWI_ID%TYPE,
							p_nowa_DATA_UZYSKANIA in SZACHOWE_LICENCJE.SZA_DATA_UZYSKANIA%TYPE)
is
	--OTWARCIE KURSORA
	CURSOR kursor_SZACHOWE_LICENCJE2(k2__SZ_KAT_ID in SZACHOWE_LICENCJE.KAT_ID%TYPE, 
							k2_SZ_ZWI_ID in SZACHOWE_LICENCJE.ZWI_ID%TYPE,
							k2_SZ_OSO_ID in SZACHOWE_LICENCJE.OSO_ID%TYPE)
	is
		select * from SZACHOWE_LICENCJE 
		where KAT_ID = k2__SZ_KAT_ID and 
			ZWI_ID = k2_SZ_ZWI_ID and
			OSO_ID=k2_SZ_OSO_ID;
	--TWORZENIE ZMIENNEJ O TYPIE WIERSZOWYM
	dawny_wiersz kursor_SZACHOWE_LICENCJE2%ROWTYPE;
	--TWORZENIE REKORDU POMOCNICZEGO
	type POMDATE is record(
		status number(3) default 0,
		opis_poprzedni string(40),
		opis_obecny string(40),
		opis_pom string(40) default 'BLAD W DODAWANIU OPISU')
		;
	--TWORZENIE REKORDU DANYCH
	type DANE is record(
		prev_SZ_KAT_ID SZACHOWE_LICENCJE.KAT_ID%TYPE,
		prev_SZ_ZWI_ID SZACHOWE_LICENCJE.ZWI_ID%TYPE,
		prev_OPIS POMDATE);
	--UTWORZENIE ZMIENNEJ O TYPIE "REKORD DANE"
	prev_rekord DANE;
	--DODAJE DYREKTYWE AUTONOMICZNYCH TRANSAKCJI
	PRAGMA AUTONOMOUS_TRANSACTION;
	--TWORZENIE WYJATKU
	EX_UI EXCEPTION;
	PRAGMA EXCEPTION_INIT(EX_UI,-20001);
begin
	--OTWARCIE KURSORA
	open kursor_SZACHOWE_LICENCJE2(p__SZ_KAT_ID, p_SZ_ZWI_ID,p_SZ_OSO_ID);
	--PETLA
	loop 
	--PRZESZUKIWANIE KURSORA I ZAPISYWANIE WYNIKOW DO ZMIENNEJ dawny_wiersz
	fetch kursor_SZACHOWE_LICENCJE2 into dawny_wiersz;
		-- warunek wyjścia z pętli
		exit when kursor_SZACHOWE_LICENCJE2%NOTFOUND;
		--USTAWIANIE WARTOSCI ZMIENNYCH NA PODSTAWIE DANWEGO REKORDU
		prev_rekord.prev_SZ_KAT_ID := dawny_wiersz.KAT_ID;
		prev_rekord.prev_SZ_ZWI_ID := dawny_wiersz.ZWI_ID;
		--WYWOLANIE FUNKCJI W CELU USTAWIENIU OPISU
		prev_rekord.prev_OPIS.opis_poprzedni := f_SZA_OPIS_OPERACJI(p__SZ_KAT_ID, p_SZ_ZWI_ID,p_SZ_OSO_ID);
		--UPDATE WSZYTSKICH POL POZA OPISEM
		update SZACHOWE_LICENCJE set KAT_ID = p_nowa_KAT_ID,
			ZWI_ID = p_nowa_ZWI_ID,
			SZA_DATA_UZYSKANIA = p_nowa_DATA_UZYSKANIA
			where
			KAT_ID = prev_rekord.prev_SZ_KAT_ID and
			OSO_ID =p_SZ_OSO_ID and
			ZWI_ID = prev_rekord.prev_SZ_ZWI_ID;
			--SPRAWDZENIE UPDATE
			if SQL%FOUND then
				commit;
			end if;
		--PONOWNE WYWOLANIE FUNKCJI W CELU UUATUALNIENIU OPISU
		prev_rekord.prev_OPIS.opis_pom := f_SZA_OPIS_OPERACJI(p_nowa_KAT_ID,p_nowa_ZWI_ID,p_SZ_OSO_ID);
		--UPDATE OPISU
		update SZACHOWE_LICENCJE set 
			SZA_OPIS = prev_rekord.prev_OPIS.opis_pom
			where
			KAT_ID = p_nowa_KAT_ID and
			OSO_ID =p_SZ_OSO_ID and
			ZWI_ID = p_nowa_ZWI_ID;
		--USTAWIANIE WARTOSCI ZMIENNYCH NA PODSTAWIE DANWEGO REKORDU
		prev_rekord.prev_OPIS.status := prev_rekord.prev_OPIS.status + 1;
		prev_rekord.prev_OPIS.opis_obecny := f_SZA_OPIS_OPERACJI(p_nowa_KAT_ID,p_nowa_ZWI_ID,p_SZ_OSO_ID);
		--WYSWIETLENIE INFORMACJI O DOKONANYCH ZMIANACH
		dbms_output.put_line('Wykonano UPDATE '||prev_rekord.prev_OPIS.status);
		dbms_output.put_line('STARY OPIS: '||prev_rekord.prev_OPIS.opis_poprzedni);
		dbms_output.put_line('NOWY OPIS: '||prev_rekord.prev_OPIS.opis_obecny);
	end loop;
	--ZAMKNIECIE KURSORA
	close kursor_SZACHOWE_LICENCJE2;
	--SPRAWDZENIE OPREACJI
	if prev_rekord.prev_OPIS.status = 0 then
		--DODANIE NOWEGO REKORDU
		insert into SZACHOWE_LICENCJE(SZA_DATA_UZYSKANIA, OSO_ID, KAT_ID, ZWI_ID)
		values (p_nowa_DATA_UZYSKANIA ,p_SZ_OSO_ID, p_nowa_KAT_ID, p_nowa_ZWI_ID);
		--SPRAWDZENIE BLEDU
		if SQL%NOTFOUND then
				ROLLBACK;
				RAISE_APPLICATION_ERROR(-20001,'BLAD WYKONANIA OBU OPERACJI');
		else
			--ZATWIERDZENIE
			commit;
			...
			update SZACHOWE_LICENCJE set 
				...
				ZWI_ID = p_nowa_ZWI_ID;
			--WYSWIETLENIE INFORMACJI O DOKONANYCH ZMIANACH
			dbms_output.put_line('Utworzono rekord: '||prev_rekord.prev_OPIS.opis_obecny);
				
		end if;
	end if;
	--ZATWIERDZENIE
	commit;
--POSTEPOWANIE GDY WYKRYTY ZOSTAJE BLAD
exception
	when EX_UI then
		DBMS_OUTPUT.PUT_LINE('p_UPorINS_SZA_LICENCJE - Wykryto błąd: '||SQLERRM);
	when OTHERS then
		DBMS_OUTPUT.PUT_LINE('Nieokreslony błąd w procedurze p_UPorINS_SZA_LICENCJE!');
		DBMS_OUTPUT.PUT_LINE(DBMS_UTILITY.FORMAT_ERROR_STACK);
		DBMS_OUTPUT.PUT_LINE(DBMS_UTILITY.FORMAT_CALL_STACK);
end;
/

--WYSWIETLENIE SACHOWYCH LICENCJI
SELECT SZA_ID,KAT_ID,ZWI_ID,OSO_ID,SZA_DATA_UZYSKANIA,SZA_OPIS 
FROM SZACHOWE_LICENCJE;
/*

    SZA_ID     KAT_ID     ZWI_ID     OSO_ID SZA_DATA_UZYSKAN SZA_OPIS
---------- ---------- ---------- ---------- ---------------- ----------------------------------------
         1          1          1          1 2016-03-01 00:00
         2          2          2          2 2015-03-01 00:00
         3          3          3          3 2013-07-01 00:00 KATEGORIA A
         4          4          4          4 2011-04-01 00:00 KATEGORIA A
         5          5          5          5 2011-03-02 00:00 KATEGORIA B+
         6          3          2          6 2019-12-01 00:00 KATEGORIA A
         7          6          3          7 2015-03-01 00:00 WYDAL ZWIAZEK O ID 3
         8          6          3          8 2013-03-01 00:00 WYDAL ZWIAZEK O ID 3

*/


--WYKONA SIE DODANIE NOWEJ LICENCJI
EXEC p_UPorINS_SZA_LICENCJE(1,2,12,3,1,'2020/06/10');
/*
Dodano nowy wiersz do SZACHOWE_LICENCJE - SZA_ID=9
Utworzono rekord: OSO:12/ZWI:1/KAT:3
*/
...
---------- ---------- ---------- ---------- ---------------- ----------------------------------------
         ...
         7          6          3          7 2015-03-01 00:00 WYDAL ZWIAZEK O ID 3
         8          6          3          8 2013-03-01 00:00 WYDAL ZWIAZEK O ID 3
         9          3          1         12 2020-06-10 00:00 OSO:12/ZWI:1/KAT:3

*/

--WYKONA SIE UPDATE NOWEJ LICENCJI
EXEC p_UPorINS_SZA_LICENCJE(3,1,12,2,2,'2020/06/10');
/*

Wykonano UPDATE 1
STARY OPIS: OSO:12/ZWI:1/KAT:3
NOWY OPIS: OSO:12/ZWI:2/KAT:2
...
---------- ---------- ---------- ---------- ---------------- ----------------------------------------
         ...
         7          6          3          7 2015-03-01 00:00 WYDAL ZWIAZEK O ID 3
         8          6          3          8 2013-03-01 00:00 WYDAL ZWIAZEK O ID 3
         9          2          2         12 2020-06-10 00:00 OSO:12/ZWI:2/KAT:2
...
PROMPT----------------------------------------;
PROMPT----------------------------------------;
PROMPT 				kolekcje pl/sql;
PROMPT----------------------------------------;

--FORMATOWANIE
CLEAR COLUMNS;
column V_IMIE HEADING 'IMIE' FORMAT a20 WRAP
column V_NAZWISKO HEADING 'NAZWISKO' FORMAT a20 WRAP
column V_PESEL HEADING 'PESEL' format a11 WRAP
column V_MIEJSCOWOSC HEADING 'MIEJSCOWOSC' format A20 WRAP
column V_KOD HEADING 'KOD POCZTOWY' format A10 WRAP
column V_ULICA HEADING 'ULICA' format A20 WRAP
column V_NUMER_BUDYNKU HEADING 'NR DOMU' format A10 WRAP
column V_NUMER_MIESZK HEADING 'NR DOMU' format A10 WRAP

--TWORZENIE PERSPEKTYWY  DO WPROWADZANIA DANYCH DO TABEL ADRESY, OSOBY [V_DANE_ADRESOWE_OSOBY]
CREATE OR REPLACE VIEW V_DANE_ADRESOWE_OSOBY
(V_IMIE,V_NAZWISKO,V_MIEJSCOWOSC,V_KOD,V_ULICA,V_NUMER_BUDYNKU,V_NUMER_MIESZK)
AS
SELECT o.OSO_IMIE1,o.OSO_NAZWISKO,a.ADR_MIEJSCOWOSC,a.ADR_KOD_POCZTOWY,a.ADR_ULICA,a.ADR_NR_DOMU,a.ADR_NR_MIESZKANIA FROM OSOBY o,ADRESY a
WHERE o.ADR_ID=a.ADR_ID;

--WYSWIETLENIE PERSPEKTYWY
SELECT * 
FROM V_DANE_ADRESOWE_OSOBY;
/*


IMIE                 NAZWISKO             MIEJSCOWOSC          KOD POCZTO ULICA                NR DOMU    NR DOMU
-------------------- -------------------- -------------------- ---------- -------------------- ---------- ----------
JAN                  KUSEK                Debica               39-200     Krakowska            13A
ANDRZEJ              KING                 Krakow               31-878     Orlinskiego          12b        13
KAMIL                KRAWCZYK             Debica               39-200     Krakowska            13A
MACIEK               KLANOWY              Krakow               31-878     Orlinskiego          12b        13
PAWEL                JUMPER               Warszawa             00-001     Krakowska            115A
KAMIL                MROCZKA              Warszawa             00-002     Rzeszowska           11         75
ADAM                 KRAWCZYK             Debica               39-200     Fredry               73
TOMASZ               KLANOWY              Gi??ycko             11-500     Orlinskiego          10         99
TOMASZ               SKOCZEK              Gi??ycko             11-500     Orlinskiego          10         99
AGATA                MROCZKA              Warszawa             00-001     Krakowska            115A
MARIOLA              KRAWCZYK             Debica               39-200     Fredry               73
WIES??AW             KLANOWY              Krakow               31-878     Orlinskiego          12b        13
PAWE??               TOCZYNSKI            Warszawa             00-001     Fredry               115A
KAMIL                MROCZKA              Warszawa             00-002     Kolakowska           11         75


*/


--TWORZENIE PROCEDURY  DO WPROWADZANIA DANYCH DO TABEL ADRESY, OSOBY [INSERT_V_DANE_ADRESOWE]
create or replace procedure INSERT_V_DANE_ADRESOWE(p_IMIE in varchar2, p_NAZWISKO in varchar2, p_MIEJSCOWOSC in varchar2, p_KOD in varchar2, p_ULICA in varchar2, p_NUMER_BUDYNKU in varchar2, p_NUMER_MIESZKANIA in varchar2)
is
--UTWORZENIE ZMIENNYCH POMOCNICZYCH
pesel number;
peselCHAR varchar(11);
pomocnicza number;
adres_id OSOBY.ADR_ID%TYPE;
--DODAJE DYREKTYWE AUTONOMICZNYCH TRANSAKCJI
PRAGMA AUTONOMOUS_TRANSACTION;
begin
	--TWORZENIE PUNKTU KONTROLNEGO
	SAVEPOINT insert_kurs;
	--USTAWIANIE LOSOWEJ WARTOSCI PESELA ZA POMOCA DBMS_RANDOM
	pesel := DBMS_RANDOM.VALUE(1000000, 9999999);
	--POBRANIE ID OSOBY OSTATNIEJ
	select SEQ_OSOBY.currval into pomocnicza 
	from dual;
	--DODANIE UNIKALNOSCI DO PESELU
	pesel := pesel+pomocnicza;
	--INSERT DO TABELI ADRESY I POBIERAM ZA POMOCĄ returning ZAWARTOSC POLA ADR_ID
	insert into ADRESY (ADR_MIEJSCOWOSC,ADR_KOD_POCZTOWY, ADR_ULICA, ADR_NR_DOMU,ADR_NR_MIESZKANIA)
	values(p_MIEJSCOWOSC, p_KOD, p_ULICA, p_NUMER_BUDYNKU, p_NUMER_MIESZKANIA) returning ADR_ID into adres_id;
	--SPRAWDZENIE
	if SQL%NOTFOUND then
		--POSTEPOWANIE GDY WYNIKNIE BLAD
		ROLLBACK TO SAVEPOINT insert_kurs;
		raise_application_error(-20000, 'Nie udalo sie utworzyc wiersza');
	end if;
	--ZMIANA WARTOSCI LICZBOWEJ PESELU NA TEKSTOWĄ
	peselCHAR:= TO_CHAR(pesel,'9999999');
	--EWENTUALNE USUPELNIENIE WARTOSCI PESELU
	peselCHAR := LPAD(peselCHAR,11,'0');
	--INSERT DO TABELI ADRESY Z DODANIEM PESELU WCZESNIEJ UTWORZONEGO
	insert into OSOBY (OSO_IMIE1, OSO_NAZWISKO, OSO_PESEL, ADR_ID)
	values(p_imie, p_nazwisko, peselCHAR, adres_id);
	--ZATWIERDZENIE
	commit;
end;
/
...
--WYWOŁANIE PROCEDURY
EXEC INSERT_V_DANE_ADRESOWE('JAN','KUSEK','DEBICA','39-200','FREDRY','17','33');
...
PAWE??               TOCZYNSKI            Warszawa             00-001     Fredry               115A
KAMIL                MROCZKA              Warszawa             00-002     Kolakowska           11         75
JAN                  KUSEK                DEBICA               39-200     FREDRY               17         33

*/
--DZIALANIE JEST PRAWIDLOWE


--PROCEDURA DODAJACA DANA ILOSC DANYCH
create or replace procedure p_OSO_ADRESY_DODAJ(liczbaRekordow IN number)
is
	--TWORZENIE ZMIENNYCH TYPU TABLICOWEGO
	type TIMIE is VARRAY(256) of VARCHAR2(50);
	type TNAZWISKO is VARRAY(256) of VARCHAR2(50);
	type TMIASTO is VARRAY(128) of VARCHAR2(255);
	type TULICA is VARRAY(128) of VARCHAR2(255);
	--TWORZENIE ZMIENNYCH POMOCNICZYCH
	kodN1 number;
	kodN2 number;
	n_IMIE V_DANE_ADRESOWE_OSOBY.V_IMIE%TYPE;
	n_NAZWISKO V_DANE_ADRESOWE_OSOBY.V_NAZWISKO%TYPE;
	n_MIEJSCOWOSC V_DANE_ADRESOWE_OSOBY.V_MIEJSCOWOSC%TYPE;
	n_KOD V_DANE_ADRESOWE_OSOBY.V_KOD%TYPE;
	n_ULICA V_DANE_ADRESOWE_OSOBY.V_ULICA%TYPE;
	n_NUMER_BUDYNKU V_DANE_ADRESOWE_OSOBY.V_NUMER_BUDYNKU%TYPE;
	n_NUMER_MIESZKANIA V_DANE_ADRESOWE_OSOBY.V_NUMER_MIESZK%TYPE;
	
	--DEFINICJA TABLIC
	lista_miast TMIASTO := TMIASTO ('Aleksandrów Kujawski','Aleksandrów Łódzki','Alwernia','Andrespol','Andrychów','Annopol','Augustów','Babice','Nędza','Babimost','Baborów','Bąków','Strumień','Baranów Sandomierski','Barcin','Barczewo','Bardo','Barlinek','Bartoszyce','Barwice','Bażany','Kluczbork','Bedoń Wieś','Andrespol','Będzin','Bełk','Warszawa', 'Krakow', 'Lodz', 'Wroclaw', 'Poznan', 'Gdansk', 'Szczecin', 'Bydgoszcz', 'Lublin', 'Bialystok', 'Katowice', 'Gdynia', 'Czestochowa', 'Radom', 'Torun', 'Sosnowiec', 'Kielce', 'Rzeszow', 'Gliwice', 'Zabrze', 'Olsztyn', 'Bielsko-Biala', 'Bytom', 'Zielona Gora', 'Rybnik', 'Ruda Slaska', 'Opole', 'Tychy', 'Gorzow Wielkopolski', 'Plock', 'Dabrowa Gornicza', 'Elblag', 'Walbrzych', 'Wloclawek', 'Tarnow', 'Chorzow', 'Koszalin', 'Kalisz');
	lista_ulic TULICA := TULICA ('Antoniny', 'Antonińska', 'Aptekarska', 'Armii Krajowej', 'Artyleryjska', 'AsnykaAdama', 'Akacjowa', 'Bajkowa', 'BatoregoStefana', 'Bema Józefagen.', 'Berwińskich', 'Bilewiczówny Oleńki', 'Boczna', 'Bohaterów Westerplatte', 'Bogusławskiego Wojciecha', 'Borowikowa', 'Borówkowa', 'Bracka', 'Brata Alberta', 'Broniewskiego Władysława', 'Brzechwy Jana', 'Brzozowa', 'Budowlanych', 'Bułgarska', 'Belgijska', 'ChełmońskiegoJózefa', 'Chocimska', 'Chociszewskiego Józefa', 'Choinkowa', 'Chopina Fryderyka', 'Zygmunta Starego', 'Zwycięstwa', 'Żeromskiego Stefana', 'Żeleńskiego Boya Tadeusza', 'Żniwna', 'Żołnierska', 'Żółkiewskiego Stanisława', 'Żwirkii Wigury','Kopalina', 'Krolewska', 'Gdynska', 'Wielkotyrnowska', 'Gramatyka', 'Jantarowa', 'Lisia', 'Rzepichy', 'Pylna', 'Bliska', 'Orla', 'Klonowa', 'Przyjemna', 'Cyganska', 'Na Wirach', 'Ukryta', 'Litewska', 'Skalna', 'Rybna', 'Bagatela', '6 Sierpnia', 'Tatarska', 'Dolina', 'Towarowa', 'Jeczmienna');
	lista_imion TIMIE := TIMIE('Aleksandra','Natalia','Karolina','Patrycja','Kinga','Klaudia','Magdalena','Julia','Gabriela','Dominika','Justyna','Paulina','Weronika','Angelika','Sylwia','Anita','Zuzanna','Alicja','Kamila','Oliwia','Kamil','Jakub','Patryk','Adrian','Michał','Dawid',' Kacper','Mateusz','Karol','Krzysztof','Zuzanna', 'Julia', 'Maja', 'Zofia', 'Hanna', 'Lena', 'Maria', 'Alicja', 'Amelia', 'Oliwia', 'Antoni', 'Jakub', 'Jan', 'Szymon', 'Aleksander', 'Franciszek', 'Filip', 'Wojciech', 'Mikołaj', 'Kacper', 'Barbara', 'Józefa', 'Nadzieja', 'Maja', 'Róża', 'Pola', 'Marianna', 'Klara', 'Kazimierz', 'Franciszek', 'Henryk', 'Julian', 'Wit', 'Gustaw', 'Adam', 'Ryszard');
	lista_nazwisk TNAZWISKO := TNAZWISKO ('Nowak', 'Kowalski', 'Wisniewski', 'Wojcik', 'Kowalczyk', 'Kaminski', 'Lewandowski', 'Zielinski', 'Wozniak', 'Szymanski', 'Dabrowski', 'Kozlowski', 'Mazur', 'Jankowski', 'Kwiatkowski', 'Wojciechowski', 'Krawczyk', 'Kaczmarek', 'Piotrowski', 'Grabowski');
	
	--TWORZENIE POMOCNICZYCH INDEXOW TABLIC
	ix_miasto number;
	ix_ulica number;
	ix_imie number;
	ix_nazwisko number;	
...
begin	
	...
	--DEFINICJA PETLI FOR
	for var_i in 1 .. liczbaRekordow
	loop
		--LOSOWANIE INDEXOW
		ix_miasto := floor(DBMS_RANDOM.VALUE(1,lista_miast.COUNT+0.9));
		ix_ulica := floor(DBMS_RANDOM.VALUE(1,lista_ulic.COUNT+0.9));
		ix_imie := floor(DBMS_RANDOM.VALUE(1,lista_imion.COUNT+0.9));
		ix_nazwisko := floor(DBMS_RANDOM.VALUE(1,lista_nazwisk.COUNT+0.9));
		--WYWOLANIE ELEMENTOW TABLIC NA PODSTAWIE INDEXOW
		n_MIEJSCOWOSC := lista_miast(ix_miasto);
		n_ULICA := lista_ulic(ix_ulica);
		n_IMIE := lista_imion(ix_imie);
		n_NAZWISKO := lista_nazwisk(ix_nazwisko);
		--LOSOWANIE FRAGMENTOW KODU
		kodN1 := DBMS_RANDOM.VALUE(10, 99);
		kodN2 := DBMS_RANDOM.VALUE(100, 990);
		--LACZENIE FRAGMENTOW W KOD POCZTOWY
		n_KOD := CONCAT(TO_CHAR(kodN1,'99'),'-');
		n_KOD := CONCAT(n_KOD,TO_CHAR(kodN2,'999'));
		n_NUMER_BUDYNKU := floor(DBMS_RANDOM.VALUE(0, 999))||DBMS_RANDOM.string('x',1);
		n_NUMER_MIESZKANIA := floor(DBMS_RANDOM.VALUE(0, 999));
		--WYKONANIE PROCEDURY WSTAWIAJACEJ DANE
		INSERT_V_DANE_ADRESOWE(n_IMIE, n_NAZWISKO, n_MIEJSCOWOSC, n_KOD, n_ULICA, n_NUMER_BUDYNKU, n_NUMER_MIESZKANIA);
	end loop;
	--ZATWIERDZENIE
	commit;
--OBSLUGA BLEDOW
exception
	when OTHERS then
		DBMS_OUTPUT.PUT_LINE('Nieokreslony błąd w procedurze proc_DODAJ_DANE_ADRESOWE!');
		DBMS_OUTPUT.PUT_LINE(DBMS_UTILITY.FORMAT_ERROR_STACK);
		DBMS_OUTPUT.PUT_LINE(DBMS_UTILITY.FORMAT_CALL_STACK);
end;
/
...
--WYWOLANIE DODANIA 100 NOWYCH REKORDOW
EXEC p_OSO_ADRESY_DODAJ(100);
/*
Dodano nowy wiersz do SEZON - ADR_ID=16
Dodano nowy wiersz do SEZON - OSO_ID=16
Dodano nowy wiersz do SEZON - ADR_ID=17
Dodano nowy wiersz do SEZON - OSO_ID=17
Dodano nowy wiersz do SEZON - ADR_ID=18
Dodano nowy wiersz do SEZON - OSO_ID=18
Dodano nowy wiersz do SEZON - ADR_ID=19
Dodano nowy wiersz do SEZON - OSO_ID=19
Dodano nowy wiersz do SEZON - ADR_ID=20
Dodano nowy wiersz do SEZON - OSO_ID=20
Dodano nowy wiersz do SEZON - ADR_ID=21
Dodano nowy wiersz do SEZON - OSO_ID=21
Dodano nowy wiersz do SEZON - ADR_ID=22
Dodano nowy wiersz do SEZON - OSO_ID=22
Dodano nowy wiersz do SEZON - ADR_ID=23
Dodano nowy wiersz do SEZON - OSO_ID=23
Dodano nowy wiersz do SEZON - ADR_ID=24
Dodano nowy wiersz do SEZON - OSO_ID=24
Dodano nowy wiersz do SEZON - ADR_ID=25
Dodano nowy wiersz do SEZON - OSO_ID=25
Dodano nowy wiersz do SEZON - ADR_ID=26
Dodano nowy wiersz do SEZON - OSO_ID=26
Dodano nowy wiersz do SEZON - ADR_ID=27
Dodano nowy wiersz do SEZON - OSO_ID=27
Dodano nowy wiersz do SEZON - ADR_ID=28
Dodano nowy wiersz do SEZON - OSO_ID=28
Dodano nowy wiersz do SEZON - ADR_ID=29
Dodano nowy wiersz do SEZON - OSO_ID=29
Dodano nowy wiersz do SEZON - ADR_ID=30
Dodano nowy wiersz do SEZON - OSO_ID=30
Dodano nowy wiersz do SEZON - ADR_ID=31
Dodano nowy wiersz do SEZON - OSO_ID=31
Dodano nowy wiersz do SEZON - ADR_ID=32
Dodano nowy wiersz do SEZON - OSO_ID=32
Dodano nowy wiersz do SEZON - ADR_ID=33
Dodano nowy wiersz do SEZON - OSO_ID=33
Dodano nowy wiersz do SEZON - ADR_ID=34
Dodano nowy wiersz do SEZON - OSO_ID=34
Dodano nowy wiersz do SEZON - ADR_ID=35
Dodano nowy wiersz do SEZON - OSO_ID=35
Dodano nowy wiersz do SEZON - ADR_ID=36
Dodano nowy wiersz do SEZON - OSO_ID=36
Dodano nowy wiersz do SEZON - ADR_ID=37
Dodano nowy wiersz do SEZON - OSO_ID=37
Dodano nowy wiersz do SEZON - ADR_ID=38
Dodano nowy wiersz do SEZON - OSO_ID=38
Dodano nowy wiersz do SEZON - ADR_ID=39
Dodano nowy wiersz do SEZON - OSO_ID=39
Dodano nowy wiersz do SEZON - ADR_ID=40
Dodano nowy wiersz do SEZON - OSO_ID=40
Dodano nowy wiersz do SEZON - ADR_ID=41
Dodano nowy wiersz do SEZON - OSO_ID=41
Dodano nowy wiersz do SEZON - ADR_ID=42
Dodano nowy wiersz do SEZON - OSO_ID=42
Dodano nowy wiersz do SEZON - ADR_ID=43
Dodano nowy wiersz do SEZON - OSO_ID=43
Dodano nowy wiersz do SEZON - ADR_ID=44
Dodano nowy wiersz do SEZON - OSO_ID=44
Dodano nowy wiersz do SEZON - ADR_ID=45
Dodano nowy wiersz do SEZON - OSO_ID=45
Dodano nowy wiersz do SEZON - ADR_ID=46
Dodano nowy wiersz do SEZON - OSO_ID=46
Dodano nowy wiersz do SEZON - ADR_ID=47
Dodano nowy wiersz do SEZON - OSO_ID=47
Dodano nowy wiersz do SEZON - ADR_ID=48
Dodano nowy wiersz do SEZON - OSO_ID=48
Dodano nowy wiersz do SEZON - ADR_ID=49
Dodano nowy wiersz do SEZON - OSO_ID=49
Dodano nowy wiersz do SEZON - ADR_ID=50
Dodano nowy wiersz do SEZON - OSO_ID=50
Dodano nowy wiersz do SEZON - ADR_ID=51
Dodano nowy wiersz do SEZON - OSO_ID=51
Dodano nowy wiersz do SEZON - ADR_ID=52
Dodano nowy wiersz do SEZON - OSO_ID=52
Dodano nowy wiersz do SEZON - ADR_ID=53
Dodano nowy wiersz do SEZON - OSO_ID=53
Dodano nowy wiersz do SEZON - ADR_ID=54
Dodano nowy wiersz do SEZON - OSO_ID=54
Dodano nowy wiersz do SEZON - ADR_ID=55
Dodano nowy wiersz do SEZON - OSO_ID=55
Dodano nowy wiersz do SEZON - ADR_ID=56
Dodano nowy wiersz do SEZON - OSO_ID=56
Dodano nowy wiersz do SEZON - ADR_ID=57
Dodano nowy wiersz do SEZON - OSO_ID=57
Dodano nowy wiersz do SEZON - ADR_ID=58
Dodano nowy wiersz do SEZON - OSO_ID=58
Dodano nowy wiersz do SEZON - ADR_ID=59
Dodano nowy wiersz do SEZON - OSO_ID=59
Dodano nowy wiersz do SEZON - ADR_ID=60
Dodano nowy wiersz do SEZON - OSO_ID=60
Dodano nowy wiersz do SEZON - ADR_ID=61
Dodano nowy wiersz do SEZON - OSO_ID=61
Dodano nowy wiersz do SEZON - ADR_ID=62
Dodano nowy wiersz do SEZON - OSO_ID=62
Dodano nowy wiersz do SEZON - ADR_ID=63
Dodano nowy wiersz do SEZON - OSO_ID=63
Dodano nowy wiersz do SEZON - ADR_ID=64
Dodano nowy wiersz do SEZON - OSO_ID=64
Dodano nowy wiersz do SEZON - ADR_ID=65
Dodano nowy wiersz do SEZON - OSO_ID=65
Dodano nowy wiersz do SEZON - ADR_ID=66
Dodano nowy wiersz do SEZON - OSO_ID=66
Dodano nowy wiersz do SEZON - ADR_ID=67
Dodano nowy wiersz do SEZON - OSO_ID=67
Dodano nowy wiersz do SEZON - ADR_ID=68
Dodano nowy wiersz do SEZON - OSO_ID=68
Dodano nowy wiersz do SEZON - ADR_ID=69
Dodano nowy wiersz do SEZON - OSO_ID=69
Dodano nowy wiersz do SEZON - ADR_ID=70
Dodano nowy wiersz do SEZON - OSO_ID=70
Dodano nowy wiersz do SEZON - ADR_ID=71
Dodano nowy wiersz do SEZON - OSO_ID=71
Dodano nowy wiersz do SEZON - ADR_ID=72
Dodano nowy wiersz do SEZON - OSO_ID=72
Dodano nowy wiersz do SEZON - ADR_ID=73
Dodano nowy wiersz do SEZON - OSO_ID=73
Dodano nowy wiersz do SEZON - ADR_ID=74
Dodano nowy wiersz do SEZON - OSO_ID=74
Dodano nowy wiersz do SEZON - ADR_ID=75
Dodano nowy wiersz do SEZON - OSO_ID=75
Dodano nowy wiersz do SEZON - ADR_ID=76
Dodano nowy wiersz do SEZON - OSO_ID=76
Dodano nowy wiersz do SEZON - ADR_ID=77
Dodano nowy wiersz do SEZON - OSO_ID=77
Dodano nowy wiersz do SEZON - ADR_ID=78
Dodano nowy wiersz do SEZON - OSO_ID=78
Dodano nowy wiersz do SEZON - ADR_ID=79
Dodano nowy wiersz do SEZON - OSO_ID=79
Dodano nowy wiersz do SEZON - ADR_ID=80
Dodano nowy wiersz do SEZON - OSO_ID=80
Dodano nowy wiersz do SEZON - ADR_ID=81
Dodano nowy wiersz do SEZON - OSO_ID=81
Dodano nowy wiersz do SEZON - ADR_ID=82
Dodano nowy wiersz do SEZON - OSO_ID=82
Dodano nowy wiersz do SEZON - ADR_ID=83
Dodano nowy wiersz do SEZON - OSO_ID=83
Dodano nowy wiersz do SEZON - ADR_ID=84
Dodano nowy wiersz do SEZON - OSO_ID=84
Dodano nowy wiersz do SEZON - ADR_ID=85
Dodano nowy wiersz do SEZON - OSO_ID=85
Dodano nowy wiersz do SEZON - ADR_ID=86
Dodano nowy wiersz do SEZON - OSO_ID=86
Dodano nowy wiersz do SEZON - ADR_ID=87
Dodano nowy wiersz do SEZON - OSO_ID=87
Dodano nowy wiersz do SEZON - ADR_ID=88
Dodano nowy wiersz do SEZON - OSO_ID=88
Dodano nowy wiersz do SEZON - ADR_ID=89
Dodano nowy wiersz do SEZON - OSO_ID=89
Dodano nowy wiersz do SEZON - ADR_ID=90
Dodano nowy wiersz do SEZON - OSO_ID=90
Dodano nowy wiersz do SEZON - ADR_ID=91
Dodano nowy wiersz do SEZON - OSO_ID=91
Dodano nowy wiersz do SEZON - ADR_ID=92
Dodano nowy wiersz do SEZON - OSO_ID=92
Dodano nowy wiersz do SEZON - ADR_ID=93
Dodano nowy wiersz do SEZON - OSO_ID=93
Dodano nowy wiersz do SEZON - ADR_ID=94
Dodano nowy wiersz do SEZON - OSO_ID=94
Dodano nowy wiersz do SEZON - ADR_ID=95
Dodano nowy wiersz do SEZON - OSO_ID=95
Dodano nowy wiersz do SEZON - ADR_ID=96
Dodano nowy wiersz do SEZON - OSO_ID=96
Dodano nowy wiersz do SEZON - ADR_ID=97
Dodano nowy wiersz do SEZON - OSO_ID=97
Dodano nowy wiersz do SEZON - ADR_ID=98
Dodano nowy wiersz do SEZON - OSO_ID=98
Dodano nowy wiersz do SEZON - ADR_ID=99
Dodano nowy wiersz do SEZON - OSO_ID=99
Dodano nowy wiersz do SEZON - ADR_ID=100
Dodano nowy wiersz do SEZON - OSO_ID=100
Dodano nowy wiersz do SEZON - ADR_ID=101
Dodano nowy wiersz do SEZON - OSO_ID=101
Dodano nowy wiersz do SEZON - ADR_ID=102
Dodano nowy wiersz do SEZON - OSO_ID=102
Dodano nowy wiersz do SEZON - ADR_ID=103
Dodano nowy wiersz do SEZON - OSO_ID=103
Dodano nowy wiersz do SEZON - ADR_ID=104
Dodano nowy wiersz do SEZON - OSO_ID=104
Dodano nowy wiersz do SEZON - ADR_ID=105
Dodano nowy wiersz do SEZON - OSO_ID=105
Dodano nowy wiersz do SEZON - ADR_ID=106
Dodano nowy wiersz do SEZON - OSO_ID=106
Dodano nowy wiersz do SEZON - ADR_ID=107
Dodano nowy wiersz do SEZON - OSO_ID=107
Dodano nowy wiersz do SEZON - ADR_ID=108
Dodano nowy wiersz do SEZON - OSO_ID=108
Dodano nowy wiersz do SEZON - ADR_ID=109
Dodano nowy wiersz do SEZON - OSO_ID=109
Dodano nowy wiersz do SEZON - ADR_ID=110
Dodano nowy wiersz do SEZON - OSO_ID=110
Dodano nowy wiersz do SEZON - ADR_ID=111
Dodano nowy wiersz do SEZON - OSO_ID=111
Dodano nowy wiersz do SEZON - ADR_ID=112
Dodano nowy wiersz do SEZON - OSO_ID=112
Dodano nowy wiersz do SEZON - ADR_ID=113
Dodano nowy wiersz do SEZON - OSO_ID=113
Dodano nowy wiersz do SEZON - ADR_ID=114
Dodano nowy wiersz do SEZON - OSO_ID=114
Dodano nowy wiersz do SEZON - ADR_ID=115
Dodano nowy wiersz do SEZON - OSO_ID=115
*/
...
KAMIL                MROCZKA              Warszawa             00-002     Kolakowska           11         75
JAN                  KUSEK                DEBICA               39-200     FREDRY               17         33
Marianna             Kaczmarek            Wloclawek             86- 112   Bu?garska            6784       637
Oliwia               Krawczyk             Katowice              11- 900   Zygmunta Starego     389R       850
Sylwia               Kwiatkowski          Rybnik                78- 300   6 Sierpnia           944O       390
Kamil                Grabowski            Strumie?              63- 559   Antoniny             160        491
R??a                 Jankowski            Alwernia              76- 949   Budowlanych          656X       421
Miko?aj              Piotrowski           Katowice              49- 718   ?eromskiego Stefana  594S       561
Gabriela             Piotrowski           B?dzin                55- 253   Litewska             8L         832
Gabriela             Jankowski            Kluczbork             82- 572   Chociszewskiego J?ze 881V       651
                                                                          fa

Angelika             Wisniewski           Barczewo              35- 657   ?eromskiego Stefana  702W       796
Kazimierz            Mazur                Bytom                 44- 587   Che?mo?skiegoJ?zefa  525T       291
Adrian               Dabrowski            Wroclaw               71- 445   Jeczmienna           51M        793
Jakub                Wojciechowski        Dabrowa Gornicza      43- 690   Na Wirach            498U       843
Aleksandra           Kwiatkowski          Kluczbork             22- 733   Orla                 982N       499
Klara                Nowak                August?w              76- 925   Brzozowa             9079       736
Miko?aj              Wisniewski           Zabrze                93- 214   ???kiewskiego Stanis 62D        458
                                                                          ?awa

Oliwia               Kaminski             Gorzow Wielkopolski   35- 630   Ukryta               701C       197
Gustaw               Jankowski            Gdansk                19- 653   Bema J?zefagen.      7618       30
Mateusz              Kozlowski            Bielsko-Biala         47- 332   Bohater?w Westerplat 8952       95
                                                                          te

Zofia                Kowalski             Wloclawek             46- 445   Na Wirach            939K       359
Mateusz              Jankowski            Babice                76- 821   Che?mo?skiegoJ?zefa  209H       352
Amelia               Kaczmarek            Barcin                82- 338   Przyjemna            117H       372
Paulina              Kwiatkowski          B?k?w                 35- 896   Gdynska              540W       241
Jakub                Wozniak              Bialystok             81- 526   Skalna               19M        80
Klara                Jankowski            Barcin                52- 758   BatoregoStefana      97M        592
Angelika             Dabrowski            Zielona Gora          67- 453   Bajkowa              293Y       596
Micha?               Grabowski            Barlinek              46- 130   Orla                 279L       513
Jakub                Kowalczyk            Alwernia              16- 686   Brzozowa             3749       330
Natalia              Szymanski            Kielce                99- 276   Broniewskiego W?adys 271X       397
                                                                          ?awa

 Kacper              Szymanski            Wroclaw               77- 656   Brata Alberta        1625       295
Gustaw               Kowalczyk            Radom                 12- 846   BatoregoStefana      784L       810
Alicja               Mazur                Barlinek              49- 335   Dolina               963N       908
Franciszek           Piotrowski           Krakow                40- 810   Pylna                961S       394
Mateusz              Kaminski             Sosnowiec             18- 716   Jeczmienna           148T       40
Kazimierz            Jankowski            Babor?w               12- 901   ?niwna               552D       101
Kazimierz            Wojcik               Krakow                90- 957   ?wirkii Wigury       934X       2
Amelia               Kowalczyk            Strumie?              34- 426   Chociszewskiego J?ze 655E       433
                                                                          fa

Weronika             Wojciechowski        Andrych?w             51- 504   BatoregoStefana      641G       276
Dawid                Kaminski             Wroclaw               60- 585   Belgijska            961C       220
Anita                Mazur                August?w              13- 487   Budowlanych          910J       75
Maja                 Kaminski             B?dzin                51- 893   Krolewska            49J        646
Kamil                Zielinski            Be?k                  73- 778   Che?mo?skiegoJ?zefa  974J       570
Barbara              Jankowski            Wroclaw               19- 190   Orla                 5693       344
Kinga                Wojciechowski        Walbrzych             75- 926   Antoni?ska           279F       822
Kacper               Dabrowski            Olsztyn               53- 822   Bu?garska            1127       173
Kamil                Krawczyk             Walbrzych             22- 527   Bu?garska            7232       394
Maria                Kwiatkowski          Krakow                95- 223   Dolina               326L       815
Henryk               Lewandowski          Chorzow               12- 939   Jantarowa            574V       892
Julia                Jankowski            Gliwice               44- 137   Zygmunta Starego     497S       842
Dawid                Zielinski            Bielsko-Biala         93- 701   Lisia                5032       131
Henryk               Kowalski             Kluczbork             92- 939   AsnykaAdama          316V       716
Maria                Kaczmarek            Bytom                 51- 821   Chocimska            11         521
Kamil                Kowalczyk            Barlinek              90- 899   Artyleryjska         372F       389
Gustaw               Krawczyk             Barczewo              42- 651   Bagatela             433S       539
Sylwia               Wozniak              Andrych?w             10- 280   Borowikowa           519E       427
Dominika             Wozniak              Poznan                55- 315   Orla                 977A       505
Nadzieja             Jankowski            August?w              84- 207   ?o?nierska           44Z        663
Kamil                Kwiatkowski          Elblag                28- 338   Artyleryjska         267T       598
Weronika             Dabrowski            Bedo? Wie?            62- 762   Ukryta               155X       98
Adrian               Lewandowski          Wloclawek             79- 904   ?ele?skiego Boya Tad 3739       394
                                                                          eusza

Zuzanna              Kwiatkowski          Baran?w Sandomierski  28- 697   Chocimska            37E        261
Gabriela             Jankowski            Katowice              84- 710   Brzechwy Jana        300Y       611
Hanna                Nowak                Kalisz                40- 704   Towarowa             971O       16
Zuzanna              Kowalski             Wloclawek             16- 588   Chopina Fryderyka    5148       933
Alicja               Kowalczyk            Gorzow Wielkopolski   60- 778   Rzepichy             215N       293
Anita                Piotrowski           Tychy                 70- 587   Budowlanych          105F       413
Klara                Dabrowski            Bardo                 24- 668   Towarowa             874N       920
Franciszek           Kaczmarek            Lublin                25- 592   Bema J?zefagen.      731Q       663
Karolina             Kaminski             Andrespol             56- 644   Przyjemna            20C        492
Angelika             Krawczyk             Bytom                 43- 108   Klonowa              3665       879
Nadzieja             Grabowski            Bydgoszcz             33- 102   Bliska               915Z       654
Anita                Szymanski            B?k?w                 52- 791   Berwi?skich          580W       870
Karol                Wisniewski           Babice                62- 904   Gdynska              1561       15
Maja                 Szymanski            Alwernia              65- 387   Bor?wkowa            809Y       228
Hanna                Kwiatkowski          Tarnow                93- 311   Boczna               556S       710
J?zefa               Lewandowski          Kielce                64- 202   Zygmunta Starego     655Y       27
Anita                Wisniewski           Andrespol             82- 810   Dolina               4891       416
Magdalena            Zielinski            Tarnow                45- 148   ?ele?skiego Boya Tad 875Z       897
                                                                          eusza

Lena                 Wojciechowski        N?dza                 42- 674   Cyganska             3785       414
Henryk               Szymanski            Czestochowa           38- 776   Ukryta               10         786
Kamil                Krawczyk             Wloclawek             66- 149   Zwyci?stwa           538N       296
J?zefa               Kozlowski            Warszawa              16- 458   Przyjemna            599M       291
Franciszek           Lewandowski          Lodz                  25- 327   AsnykaAdama          2943       409
Maja                 Kowalczyk            Warszawa              20- 787   Cyganska             8762       447
Wit                  Wojcik               Babimost              28- 692   Klonowa              260I       81
Magdalena            Jankowski            Koszalin              71- 195   Chociszewskiego J?ze 754L       917
                                                                          fa

Zuzanna              Wojciechowski        Barcin                74- 561   Tatarska             568N       635
 Kacper              Kowalczyk            Andrespol             46- 672   Bliska               7506       848
Lena                 Dabrowski            Gorzow Wielkopolski   88- 185   Bracka               9208       821
Natalia              Piotrowski           Opole                 49- 868   Akacjowa             306I       841
Alicja               Szymanski            B?k?w                 85- 446   Broniewskiego W?adys 82U        866
                                                                          ?awa

Kazimierz            Szymanski            Bedo? Wie?            59- 196   Kopalina             42E        28
Barbara              Jankowski            Lublin                49- 538   Belgijska            610P       745
Maja                 Wisniewski           Poznan                75- 720   6 Sierpnia           661M       361
Marianna             Kwiatkowski          Torun                 54- 425   Bracka               2172       307
Kazimierz            Mazur                Be?k                  54- 649   Ukryta               806V       197
J?zefa               Zielinski            Torun                 68- 182   ?o?nierska           310J       90
Szymon               Kaczmarek            Gorzow Wielkopolski   32- 132   Gramatyka            747Z       718
Alicja               Mazur                Chorzow               86- 971   Antoniny             549A       987
Weronika             Wisniewski           Barlinek              18- 350   Broniewskiego W?adys 3686       251
                                                                          ?awa

Miko?aj              Grabowski            Barlinek              41- 674   Artyleryjska         886N       583
...
*/
--PROCEDURA WYKONALA SIE POMYSLNIE

------------------------------------------------------------------------------
------------------------------------------------------------------------------
------------------------------------------------------------------------------
--WPROWADZANIE DANYCH DO 3 RÓWNYCH TABEL
------------------------------------------------------------------------------
------------------------------------------------------------------------------
------------------------------------------------------------------------------

--TWORZENIE PROCEDURY  DO WPROWADZANIA DANYCH DO TABEL SEZONY,KATEGORIE,ADRESY[proc_DODAJ_SEZ_KAT_ADR]
create or replace procedure proc_DODAJ_SEZ_KAT_ADR(ile_rekordow IN number)
is
	--TWORZENIE ZMIENNYCH TYPU TABLICOWEGO
	type TMIASTO is VARRAY(128) of VARCHAR2(255);
	type TULICA is VARRAY(128) of VARCHAR2(255);
	...
	--
	--TWORZENIE ZMIENNYCH TYPU TABLICOWEGO
	type TDATAR is VARRAY(128) of VARCHAR2(255);
	type TDATAZ is VARRAY(128) of VARCHAR2(255);
	n_DATAR varchar2(255);
	n_DATAZ varchar2(255);
	--DEFINICJA TABLIC
	lista_datr TDATAR  :=TDATAR ('2020/05/03','2020/06/03','2020/07/03','2019/05/03','2019/06/03','2019/07/03','2018/05/03','2018/06/03','2018/07/03','2017/05/03','2017/06/03','2017/07/03','2016/05/03','2016/06/03','2016/07/03','2015/05/03','2015/06/03','2015/07/03','2014/05/03','2014/06/03','2014/07/03','2013/05/03','2013/06/03','2013/07/03');
	lista_datz TDATAZ := TDATAZ('2120/05/03','2120/06/03','2120/07/03','2119/05/03','2119/06/03','2119/07/03','2118/05/03','2118/06/03','2118/07/03','2117/05/03','2117/06/03','2117/07/03','2116/05/03','2116/06/03','2116/07/03','2115/05/03','2115/06/03','2115/07/03','2114/05/03','2114/06/03','2114/07/03','2113/05/03','2113/06/03','2113/07/03');
	--TWORZENIE POMOCNICZYCH INDEXOW TABLIC
	ix_datr number;
	ix_datz number;
	...
	--TWORZENIE ZMIENNYCH POMOCNICZYCH
	pom number;
	liczba_adresow number;
	liczba_kategorii number;
	liczba_sezonow number;
	
	--DODAJE DYREKTYWE AUTONOMICZNYCH TRANSAKCJI
	PRAGMA AUTONOMOUS_TRANSACTION;
	
	
	--TWORZENIE WYJĄTKÓW O ODPOWIEDNICH NZAWACH
	EX_LICZBA_REKORDOW EXCEPTION;
		PRAGMA EXCEPTION_INIT(EX_LICZBA_REKORDOW,-20002);
	EX_ADR_INSERT EXCEPTION;
		PRAGMA EXCEPTION_INIT(EX_ADR_INSERT,-20003);
	EX_KAT_INSERT EXCEPTION;
		PRAGMA EXCEPTION_INIT(EX_KAT_INSERT,-20004);
	EX_SEZON_INSERT EXCEPTION;
		PRAGMA EXCEPTION_INIT(EX_SEZON_INSERT,-20005);
begin
	--NADANIE WARTOSCI ZMIENNEJ POM
	pom := ile_rekordow;
	--WARUNEK MIN 3 REKORDOW(SA 3 TABLICE)
	if pom < 3 then
		RAISE_APPLICATION_ERROR(-20002,'Zbyt mala ilosc rekordow do dodania! Minimum 3.');
	end if;
	--INICJOWANIE RANDOM
	DBMS_RANDOM.INITIALIZE(210399);
	--LOSOWANIE LICZBY DODANYCH DANYCH
	liczba_adresow := floor(DBMS_RANDOM.VALUE(1, pom-2));
	pom := pom - liczba_adresow;
	liczba_kategorii := floor(DBMS_RANDOM.VALUE(1, pom-1));
	liczba_sezonow := pom - liczba_kategorii;
	
	--INFOPRMACJA ILE JAKICH REKORDOW ZOSTANIE DODANYCH
	DBMS_OUTPUT.PUT_LINE('NASTAPI GENEROWANIE  '||liczba_adresow||' ADRESOW, '||liczba_kategorii||' KATEGORII oraz '||liczba_sezonow||' SEZONOW');
	
	--DODAWANIE ADRESOW
	for var_i in 1 .. liczba_adresow
	loop
		--LOSOWANIE INDEXOW
		ix_miasto := floor(DBMS_RANDOM.VALUE(1,lista_miast.COUNT+0.9));
		ix_ulica := floor(DBMS_RANDOM.VALUE(1,lista_ulic.COUNT+0.9));
		...
		n_NUMER_BUDYNKU := floor(DBMS_RANDOM.VALUE(0, 999))||DBMS_RANDOM.string('x',1);
		n_NUMER_MIESZKANIA := floor(DBMS_RANDOM.VALUE(0, 999));
		--WYKONANIE INSERTU DO TABELI ADRESY
		insert into ADRESY (ADR_MIEJSCOWOSC,ADR_KOD_POCZTOWY, ADR_ULICA, ADR_NR_DOMU,ADR_NR_MIESZKANIA)
		values(n_MIEJSCOWOSC, n_KOD, n_ULICA, n_NUMER_BUDYNKU, n_NUMER_MIESZKANIA);
	end loop;
	--ZATWIERDZENIE
	commit;
	
	--DODAWANIE KATEGORII
	for var_i in 1 .. liczba_kategorii
	loop
		--LOSOWANIE INDEXOW
		...
		--WYKONANIE INSERTU DO TABELI KATEGORIE
		insert into KATEGORIE(KAT_TYP,KAT_RANGA)
	...
	--DODAWANIE SEZONOW
	for var_i in 1 .. liczba_sezonow
	loop
		--LOSOWANIE INDEXOW
		ix_datr := floor(DBMS_RANDOM.VALUE(1,lista_datr.COUNT+0.9));
		ix_datz := floor(DBMS_RANDOM.VALUE(1,lista_datz.COUNT+0.9));
		--WYWOLANIE ELEMENTOW TABLIC NA PODSTAWIE INDEXOW
		n_DATAR := lista_datr(ix_datr);
		n_DATAZ := lista_datz(ix_datz);
		--WYKONANIE INSERTU DO TABELI SEZON
		insert into SEZON(SEZ_ROK_ROZPOCZECIA,SEZ_ROK_ZAKONCZENIA)
		values(n_DATAR,n_DATAZ);
	end loop;
	--ZATWIERDZENIE
	commit;
--OBSLUGA BŁEDÓW
exception
	when EX_SEZON_INSERT then
		DBMS_OUTPUT.PUT_LINE('proc_DODAJ_SEZ_KAT_ADR - Wykryto blad: '||SQLERRM);
	when EX_KAT_INSERT then
		DBMS_OUTPUT.PUT_LINE('proc_DODAJ_SEZ_KAT_ADR - Wykryto blad: '||SQLERRM);
	when EX_ADR_INSERT then
		DBMS_OUTPUT.PUT_LINE('proc_DODAJ_SEZ_KAT_ADR - Wykryto blad: '||SQLERRM);
	when EX_LICZBA_REKORDOW then
		DBMS_OUTPUT.PUT_LINE('proc_DODAJ_SEZ_KAT_ADR - Wykryto blad: '||SQLERRM);
	when OTHERS then
		DBMS_OUTPUT.PUT_LINE('Nieokreslony blad w procedurze proc_DODAJ_SEZ_KAT_ADR!');
		DBMS_OUTPUT.PUT_LINE(DBMS_UTILITY.FORMAT_ERROR_STACK);
		DBMS_OUTPUT.PUT_LINE(DBMS_UTILITY.FORMAT_CALL_STACK);
end;
/

--PODANIE LICZBY REKORDOW
SELECT COUNT(ADR_KOD_POCZTOWY) LA 
FROM ADRESY;
/*
 LA
----------
...
----------
       26
*/
SELECT COUNT(SEZ_ROK_ROZPOCZECIA) LS 
FROM SEZON;
/*
 LA
----------
...
EXEC proc_DODAJ_SEZ_KAT_ADR(10);
/*

NASTAPI GENEROWANIE  7 ADRESOW, 1 KATEGORII oraz 2 SEZONOW
Dodano nowy wiersz do SEZON - ADR_ID=116
Dodano nowy wiersz do SEZON - ADR_ID=117
Dodano nowy wiersz do SEZON - ADR_ID=118
Dodano nowy wiersz do SEZON - ADR_ID=119
Dodano nowy wiersz do SEZON - ADR_ID=120
Dodano nowy wiersz do SEZON - ADR_ID=121
Dodano nowy wiersz do SEZON - ADR_ID=122
Dodano nowy wiersz do SEZON - KAT_ID=27
Dodano nowy wiersz do SEZON - SEZ_ID=15
Dodano nowy wiersz do SEZON - SEZ_ID=16
